...
const Rent = {
    ...
    async singleRent(req, res) {
      ...
      if (
        ...
        return res
          ...
          .send({
            message:
              "Please provide full_payment, device_id, rent_end, rent_start, status_id, and quantity",
          });
      }
  ...
  Post: {
    async singleRent(req, res) {
      ...
      try {
        ...
        const queryInsert =
          "INSERT INTO rent (full_payment,device_id,rent_end,rent_start,status_id,quantity,user_id) VALUES (?,?,?,?,?,?,?)";
        await db
          .promise()
          .execute(queryInsert, [
            full_payment,
  ...
  Delete: {
    singleRent(req, res) {
      ...
      try {
        db.query(
          `UPDATE rent SET soft_delete = ? WHERE id = ?`,
          [soft_delete, id],
          (err, result, fields) => {
            if (err) {
              console.error(`error updating:`, err);
